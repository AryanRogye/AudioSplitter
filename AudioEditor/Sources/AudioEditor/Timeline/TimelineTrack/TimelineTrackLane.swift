...
struct TimelineTrackLane: View {
    ...
    init(clip: TimelineClip, headerWidth: CGFloat, pixelsPerSecond: CGFloat, scale: BeatScale, selected: Binding<Bool>) {
        self.clip = clip
    ...
    var body: some View {
                ...
                .gesture(
                    DragGesture(minimumDistance: 2)
                        .onChanged { value in
                            ...
                            let step = scale.secondsPerBeat / 4.0
                            let snapped = snap(raw, step: step)
                            let snapWindow = max(0.01, min(0.08, step * 0.4))
                            let useSnap = abs(snapped - raw) <= snapWindow
        ...
        .task(id: "\(clip.asset.url.absoluteString)|\(clip.sourceStart)|\(clip.duration)|\(pixelsPerSecond)") {
